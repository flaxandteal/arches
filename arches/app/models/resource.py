            ...
            if related_resources:
                ...
                to_counts = (
                    models.ResourceXResource.objects.filter(
                        to_resource_id__in=related_resource_ids
                    )
                    .values("to_resource_id")
                    .annotate(to_count=Count("to_resource_id"))
                    ...
                    .order_by(F("to_resource_id").asc(nulls_last=True))
                )
                from_counts = (
                    models.ResourceXResource.objects.filter(
                        from_resource_id__in=related_resource_ids
                    )
                    .values("from_resource_id")
                    .annotate(from_count=Count("from_resource_id"))
                    .order_by(F("from_resource_id").asc(nulls_last=True))
                )
                ...
                for related_resource_count in to_counts:
                    total_relations_by_resource_id[
                        related_resource_count["to_resource_id"]
                    ] += related_resource_count["to_count"]
                for related_resource_count in from_counts:
                    total_relations_by_resource_id[
                        related_resource_count["from_resource_id"]
